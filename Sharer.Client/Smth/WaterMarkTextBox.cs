    ...
    public class WaterMarkTextBox : TextBox {
        ...
        protected override void OnPaint(PaintEventArgs args)
        {
            ...
            SolidBrush drawBrush = new SolidBrush(WaterMarkColor);//use Water mark color
                                                                  //Draw Text or WaterMark at center
            //args.Graphics.DrawString((waterMarkTextEnabled ? WaterMarkText : Text), drawFont, drawBrush, new PointF(0.0F, 0.0F));
            SizeF watermarkSize = args.Graphics.MeasureString(_waterMarkText, drawFont);
            args.Graphics.DrawString(s: (waterMarkTextEnabled ? WaterMarkText : Text), 
                font: drawFont, 
                brush: drawBrush, 
                point: new PointF(this.Width / 2 - watermarkSize.Width / 2, 0.0F));
            base.OnPaint(args);
        }
